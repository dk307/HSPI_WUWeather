    ...
    public class ConfigPage : PageBuilder
    {
        ...
        public string PostBackProc(string data, string user, int userRights)
        {
            ...
            if (form == INV($"{IdPrefix}{SaveButtonName}"))
            {
                ...
                uint refreshIntervalMinutes;
                if (!uint.TryParse(parts[RefreshIntervalId], out refreshIntervalMinutes) || refreshIntervalMinutes > int.MaxValue)
                {
                    results.AppendLine("Refresh Interval is not Valid.<br>");
                }

                Unit unit;
                if (!System.Enum.TryParse<Unit>(parts[UnitId], out unit))
                {
